package sn.ism.gestiondettes.controllers;
...
public class DetteController {
    ...
    @Operation(summary = "Récupérer les dettes d'un client avec pagination",
            description = "Retourne les dettes d'un client avec pagination et filtre par téléphone")
    @ApiResponse(responseCode = "200", description = "Dettes du client récupérées avec pagination")
    public ResponseEntity<Page<Dette>> getDettesByClient(
            @Parameter(description = "ID du client", required = true)
            @PathVariable Long clientId,
            @Parameter(description = "Numéro de page (commence à 0)", required = false)
            @RequestParam(defaultValue = "0") int page,
            @Parameter(description = "Taille de la page", required = false)
            @RequestParam(defaultValue = "10") int size,
            @Parameter(description = "Filtre par téléphone du client", required = false)
            @RequestParam(required = false) String telephone) {

        Pageable pageable = PageRequest.of(page, size);
        Page<Dette> dettes = detteService.getDettesByClientIdWithPagination(clientId, telephone, pageable);
        return ResponseEntity.ok(dettes);
    }
    ...
    @PostMapping("/client/{clientId}/multiple")
    @Operation(summary = "Ajouter plusieurs dettes à un client", description = "Crée plusieurs nouvelles dettes pour un client")
    @ApiResponses({
            @ApiResponse(responseCode = "201", description = "Dettes créées avec succès"),
            @ApiResponse(responseCode = "400", description = "Données invalides"),
            @ApiResponse(responseCode = "404", description = "Client non trouvé")
    })
    public ResponseEntity<List<Dette>> createMultipleDettes(
            @Parameter(description = "ID du client", required = true)
            @PathVariable Long clientId,
            @Parameter(description = "Liste des dettes à créer", required = true)
            @Valid @RequestBody List<Dette> dettes) {
        try {
            List<Dette> savedDettes = detteService.createMultipleDettes(clientId, dettes);
            return ResponseEntity.status(HttpStatus.CREATED).body(savedDettes);
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
    }
