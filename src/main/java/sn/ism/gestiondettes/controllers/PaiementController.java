...
public class PaiementController {
    ...
    @Operation(summary = "Récupérer les paiements d'une dette avec pagination et filtres",
            description = "Retourne les paiements d'une dette avec pagination, filtre par téléphone du client et numéro de dette")
    @ApiResponse(responseCode = "200", description = "Paiements de la dette récupérés avec pagination")
    public ResponseEntity<Page<Paiement>> getPaiementsByDette(
            ...
            @Parameter(description = "Filtre par numéro de dette", required = false)
            @RequestParam(required = false) Long numerodette) {

        Pageable pageable = PageRequest.of(page, size);
        Page<Paiement> paiements = paiementService.getPaiementsByDetteIdWithPagination(
                detteId, telephone, numerodette, pageable);
        return ResponseEntity.ok(paiements);
    }

    @GetMapping("/dette/{detteId}/simple")
    @Operation(summary = "Récupérer les paiements d'une dette (simple)",
            description = "Retourne tous les paiements d'une dette sans pagination")
    @ApiResponse(responseCode = "200", description = "Paiements de la dette récupérés")
    public ResponseEntity<List<Paiement>> getPaiementsByDetteSimple(
            @Parameter(description = "ID de la dette", required = true)
            @PathVariable Long detteId) {
